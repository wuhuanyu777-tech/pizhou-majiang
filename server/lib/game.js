...
module.exports = class Game extends Majiang.Game {
    ...
    call_players(type, msg, timeout) {
        ...
        // 响应轮（打牌后等吃碰杠胡）：20 秒无高优先级响应时自动放弃，防止卡局
        if (type == 'dapai' && ! this._timer) {
            this._respond_timeout = setTimeout(()=>{
                if (this._status != 'dapai') return;
                if (this._reply.some(r => r && (
                        r.hule
                        || (r.fulou && r.fulou.replace(/0/g,'5')
                                    .match(/^[mpsz](\d)\1\1/))))) return;
                for (let l = 0; l < 4; l++) {
                    if (! this._reply[l]) this._reply[l] = {};
                }
                this.next();
            }, 20000);
        }
    ...
    reply(id, reply) {
        ...
        // 碰/杠/胡 为高优先级：收到即处理，无需等待低优先级（吃/放弃）玩家回复
        if (this._status == 'dapai') {
            let m = (reply.fulou || '').replace(/0/g, '5');
            let high = reply.hule
                    || (reply.fulou && m.match(/^[mpsz](\d)\1\1/));
            if (high) {
                for (let l = 0; l < 4; l++) {
                    if (! this._reply[l]) this._reply[l] = {};
                }
