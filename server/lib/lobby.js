...
class Lobby {
    ...
    enter_room(sock, room_no) {
        ...
        if (! this.ROOM[room_no]) {
            ...
            // 满 4 人自动开局（前端也会触发 START，这里作保险）
            if (this.ROOM[room_no].uids.length >= 4) {
                let host = this.USER[this.ROOM[room_no].uids[0]];
                if (host && host.sock)
                    this.start(host.sock, room_no, undefined, undefined);
            }
            this.status_log();
        }
    }
    ...
    start(sock, room_no, rule, timer) {
        ...
        console.log(`[lobby] START 收到: ${user ? user.name : '?'} / room=${room_no}`
            + ` / 房间人数=${this.ROOM[room_no] ? this.ROOM[room_no].uids.length : '房间不存在'}`);

        if (! this.ROOM[room_no]) return;
        if (user.uid != this.ROOM[room_no].uids[0]) return;
        if (this.ROOM[room_no].game) return;
        if (this.ROOM[room_no].uids.length < 4) {
            sock.emit('ERROR', `需要 4 名玩家才能开局（当前 ${this.ROOM[room_no].uids.length} 人）`);
            return;
        }
