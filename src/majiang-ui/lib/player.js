...
module.exports = class Player extends Majiang.Player {
    ...
    action_zimo(zimo, gangzimo) {
        ...
        else {                              // 坎牌后的重建消息：重绘自己手牌显示坎标记
            new Shoupai($('.shoupai.main', this._node.root),
                        this._pai, this.shoupai, true).redraw();
        }
        this.build_action_panel();
    }
    ...
    build_action_panel() {
        ...
        const gangzimo = this._gangzimo;
        const zimo     = this._zimo;
        ...
        // 坎牌按钮：有三张同牌可坎（暗刻，扣在面前，一旦坎上不可取消）
        let kanpai = this.shoupai.kanpai;
        if (kanpai.length) {
            $('.button.kan', this._node.button).text('坎')
                                               .removeClass('kan-on');
            this.set_button('kan', ()=>{
                let p = kanpai[0];
                this.shoupai.kan(p);            // 同步本地副本
                this.callback({kan: p});
            });
        }
        else {
            $('.button.kan', this._node.button)
                .removeClass('kan-on').text('坎');
        }
