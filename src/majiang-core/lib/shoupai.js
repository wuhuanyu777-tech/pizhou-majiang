...
module.exports = class Shoupai {
    ...
    constructor(qipai = []) {
        ...
        this._zimo  = null;
        this._lizhi = false;
        this._mingang = [];     // 暗坎升级的明杠面子（自摸补第4张，仍按明杠计分/显示）
    ...
    clone() {
        ...
        shoupai._mingang = this._mingang.concat();
    ...
    fromString(paistr) {
        ...
        this._mingang = shoupai._mingang.concat();

        return this;
    }

    /* 坎牌（暗刻）：三张同牌从手牌取出，扣在面前（_fulou 无方向面子） */
    kan(p) {
        p = p.slice(0,2).replace(/0/g,'5');
        let s = p[0], n = +p[1];
        if (this._bingpai[s][n] < 3)                throw new Error([this, p]);
        this._bingpai[s][n] -= 3;
        this._fulou.push(s+n+n+n);
        // 若刚摸的牌正好被坎走，把 zimo 重设到手牌中任意一张
        if (this._zimo && this._zimo.length <= 2
                && this._bingpai[this._zimo[0]][+this._zimo[1]] == 0)
        {
            this._zimo = null;
            for (let s2 of ['m','p','s','z']) {
                for (let n2 = 1; n2 < this._bingpai[s2].length; n2++) {
                    if (this._bingpai[s2][n2] > 0) { this._zimo = s2+n2; break }
                }
                if (this._zimo) break;
            }
        }
    ...
    unkan(p) {
        ...
        let s = p[0], n = +p[1];
        let m = s+n+n+n;
        let i = this._fulou.findIndex(x => x == m);
        if (i >= 0) {
            this._fulou.splice(i, 1);
            this._bingpai[s][n] += 3;
        }
        return this;
    }
    ...
    get kanpai() {
        ...
        for (let s of ['m','p','s','z']) {
            let bingpai = this._bingpai[s];
            for (let n = 1; n < bingpai.length; n++) {
                if (bingpai[n] >= 3) kanpai.push(s+n);
            }
        }
        return kanpai;
    }
    ...
    fulou(m, check = true) {
        ...
        // 暗坎开杠：别人打出第4张，暗坎（无方向3张）升级为明杠
        if (m.match(/^[mpsz]\d\d\d\d[\+\=\-]$/)) {
            let kan_m = s + m[1] + m[1] + m[1];
            let i = this._fulou.findIndex(x => x == kan_m);
            if (i >= 0) {
                this._fulou[i] = m;
                return this;
            }
    ...
    gang(m, check = true) {
        ...
        let s = m[0];
        if (m.match(/\d{4}$/)) {
            // 暗坎 + 自摸第4张：副露里有暗坎（无方向3张）→ 升级为明杠（自摸也算明杠）
            let kan_m = s + m[1] + m[1] + m[1];
            let i = this._fulou.findIndex(x => x == kan_m);
            if (i >= 0) {
                this._fulou[i] = m;
                if (! this._mingang.find(x => x == m)) this._mingang.push(m);
                this.decrease(s, m[1]);     // 扣掉手牌里的第4张
            }
            else {
                for (let n of m.match(/\d/g)) {
                    this.decrease(s, n);
    ...
    get_gang_mianzi(p) {
        ...
        if (p) {
            ...
            if (bingpai[n] == 3) {
                ...
                // 暗坎开杠：副露里有暗坎（无方向3张）与打出的牌相同 → 明杠
                for (let m of this._fulou) {
                    if (m == s+n+n+n) {
                        mianzi.push(s+n+n+n+n+d);
                        break;
                    }
                }
            }
