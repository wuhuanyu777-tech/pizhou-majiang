...
module.exports = class Player {
    ...
    action(msg, callback) {
        ...
        else if (msg.zimo && msg.zimo.kan) this.action_zimo(msg.zimo, null);
    ...
    qipai(qipai) {
        ...
        // 起手杠（塌牌）检测：配牌中有4张相同即可直接胡
        let sp = this.shoupai;
        this._qipai_gang = false;
        for (let s of ['m','p','s','z']) {
            for (let n = 1; n < sp._bingpai[s].length; n++) {
                if (sp._bingpai[s][n] == 4) this._qipai_gang = true;
            }
        }
    ...
    allow_hule(shoupai, p, hupai) {
        hupai = hupai || shoupai.lizhi || this.shan.paishu == 0
                     || (this._qipai_gang && ! p);     // 塌牌（起手杠）仅限自摸
