...
module.exports = class Player extends Majiang.Player {
    ...
    qipai(qipai) {
        ...
        this._suanpai.qipai(
            qipai, (this._id + 4 - this._model.qijia) % 4);
        super.qipai(qipai);
    }
    zimo(zimo, gangzimo) {
    ...
    action_zimo(zimo, gangzimo) {
        ...
        if      (this.select_hule(null, gangzimo))
            ...
            // AI 自动坎牌：每次行动坎一组（三张同牌不亮出，查胡按坎计分）
            let kanpai = this.shoupai.kanpai;
            if (kanpai.length) {
                this.shoupai.kan(kanpai[0]);        // 同步本地副本
                return this._callback({kan: kanpai[0]});
            }
            if      (this.select_pingju()) this._callback({daopai: '-'});
